...
/**
 * Parse reference entries from the MDT/EBM report text.
 *
 * Handles TWO distinct formats:
 *
 *   Format A — EBM deep-search report:
 *     ==================== 参考文献 (References) ====================
 *     [1] PMID: 12345678 | RCT
 *         Title: Some study title
 *         Guidelines: 前沿证据合成 (Deep Research)
 *     ----------
 *
 *   Format B — MDT scheme-generation report (plain references):
 *     ### 参考文献
 *     [1]
 ...
 *     PMID: ...
 */
function parseMdtReferences(reportText) {
  if (!reportText) return []

  // ── Step 1: locate the reference section ──
  // Match EBM format "==== ... ====", markdown "## 参考文献", bold "**参考文献**", or plain "参考文献"
  ...
  // ── Step 2: try Format A (EBM search) first ──
  ...
  // ── Step 3: fall back to Format B — MDT scheme-generation plain references ──
  // Pattern: [N]\n<Title line>\n<Source / guideline line>\nPMID: <id>
  // The Title may span multiple lines if it wraps, but typically it's one line.
  // The source line is before the PMID line.
  ...
  while ((em = plainRe.exec(refSection)) !== null) {
    ...
    // Is it a real PMID (digits only, 7-9 chars) or a content hash?
    const isRealPmid = /^\d{7,9}$/.test(pmid)
    const link = isRealPmid
      ? `https://pubmed.ncbi.nlm.nih.gov/${pmid}/`
      : ''  // hash-based refs have no resolvable link
    const source = sourceRaw || '参考文献'
    items.push({ id: items.length, title, content: title, source, link, highlighted: false, hidden: false, _fromReport: true })
  }

  // ── Step 4: even looser fallback — split on [N] boundaries and extract
  //            whatever we can (title + source + PMID per entry)
  if (items.length === 0) {
    // Remove the header line itself and any trailing "---" separators
    ...
    while ((chunk = chunkRe.exec(body)) !== null) {
      ...
      // First non-empty line after [N] is the title
      ...
      // Source = line after title, before PMID (or last non-PMID line)
...
/**
 * Fallback: extract references from evidence item PMIDs that match the report's
 * reference section. Used when parseMdtReferences returns empty but evidenceList
 * has data.
 */
function extractRefPmidsFromReport(reportText) {
...
/**
 * Structured bibliography from the MDT analysis result — the authoritative source.
 * Format: [{ ref_index, title, guidelines, pmid }, ...]
 * This is what the TreatmentView "参考文献" tab already displays.
 */
...
/** Evidence items parsed directly from the MDT report's reference section (text-based fallback). */
const mdtReferences = ref([])

/** Re-parse MDT references when the report text changes, only if structuredBib is empty. */
watch(
  () => store.mdtStreamText || store.ebmReport || '',
  (reportText) => {
    // Prefer structured bibliography; only text-parse as fallback
    if (structuredBib.value.length > 0) {
      mdtReferences.value = []
      return
    }
...
/** PMIDs cited in the MDT report's reference section (for fallback matching). */
...
/**
 * Clean MDT report text for display:
 * 1. Strip DeepSeek <think>…</think> reasoning chain
 * 2. Strip LLM conversational preamble (e.g. "好的，作为…专家，我将…")
 * 3. Remove residual "---" separators
 */
function cleanMdtReport(text) {
  if (!text) return ''
  let cleaned = text

  // Step 1: remove <think>…</think> blocks (DeepSeek reasoning)
  cleaned = cleaned.replace(/<think>[\s\S]*?<\/think>/gi, '')

  // Step 2: strip LLM conversational preamble before the actual report
  // Strategy A: look for a "---" separator line (LLMs commonly separate intro from body)
  const sepRe = /^([\s\S]*?)^---[ \t]*$/m
  ...
  if (sepMatch && sepMatch[1].length < 600 && cleaned.length - sepMatch[0].length > 60) {
    ...
    // Strategy B: find the first markdown heading that looks like a clinical section
    // Matches "# 一、" "## 1." "一、" etc.
  ...
  // Step 3: clean up any remaining "---" separator at the very beginning
...
const treatmentReportRaw = computed(() => store.mdtStreamText || '')  // unfiltered for EBM submission

// ── Cited-only evidence filter ────────────────────────────────────────────────
const citedOnly = ref(true)  // show only evidence actually referenced in MDT report
...
/** Normalize for citation matching — collapse whitespace, strip punctuation. */
function _normForCite(s) {
  return s.replace(/\s+/g, ' ').replace(/[，,。.、;；:：""''""（）\(\)\[\]【】]/g, '').trim().toLowerCase()
}

/**
 * Check if an evidence item is cited in the MDT report.
 * Uses sliding-window probes: takes 40-char snippets at 20-char steps from the
 * evidence payload, and checks if any appear verbatim in the normalized report.
 */
function _isItemCited(item, normReport) {
  ...
  const normPayload = _normForCite(payload)
  // Quick check: first 60 chars of payload
...
const citedEvidenceIds = computed(() => {
  ...
  const normReport = _normForCite(report)
  const ids = new Set()
...
/** Render EBM report as HTML — body via markdown, refs via custom HTML */
const ebmReportHtml = computed(() => {
  if (!store.ebmReport) return ''
  const { body, refsText } = splitEbmReport(store.ebmReport)
  let html = ''
  if (body) html += renderMd(body)
  if (refsText) html += renderEbmRefs(refsText)
  return html
})

/**
 * Split EBM report at the reference section.
 * Handles both "==== 参考文献 (References) ====" and "===================="
 */
function splitEbmReport(text) {
  if (!text) return { body: '', refsText: '' }
  // Match the reference section header: ======= 参考文献 ... =======
...
/**
 * Jaccard similarity on character trigrams — robust for near-duplicate detection
 * across minor wording differences (e.g. "ESGO-ESTRO-ESP guidelines" vs "ESGO/ESTRO/ESP guideline").
 */
...
/**
 * Render the reference block as clean, readable HTML (bypasses markdown).
 * Includes cross-reference deduplication: if two references have highly similar
 * detail content (Title + Guidelines), only the first is kept.
 *
 ...
 */
/**
 * Split a chunk of lines into individual reference entries.
 * Each entry starts with a "[N] ..." index line, followed by indented detail lines.
 * Returns array of {lines[]} — each a complete single-reference entry.
 */
function _splitRefEntries(lines) {
  const entries = []
  let cur = null
  for (const line of lines) {
    if (/^\[\d+\]/.test(line.trimStart())) {
      // Start of a new reference entry
...
function renderEbmRefs(refsText) {
  if (!refsText) return ''
  const esc = (s) => s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')

  // ── Pass 1: Parse all blocks into structured entries ──
  const entries = []     // { lines[], fingerprint, isDup, dupOf }
  let headerText = ''

  // Step 1a: Split on "----------" boundaries (if present), then on "[N]" boundaries
  ...
  for (const block of blocks) {
    ...
    if (/^=+/.test(firstLine)) {
      // Header line (e.g. ==================== 参考文献 ====================)
      headerText = firstLine
      // Remaining lines may contain multiple [N]-delimited refs
      const refGroups = _splitRefEntries(lines.slice(1))
      for (const refLines of refGroups) {
        const detailText = refLines.slice(1).map(l => l.trimStart()).filter(Boolean).join(' ')
        entries.push({ lines: refLines, fingerprint: _normalizeForDedup(detailText), isDup: false, dupOf: -1 })
      }
    } else {
      // No header in this block — split into individual [N] refs
      const refGroups = _splitRefEntries(lines)
      for (const refLines of refGroups) {
        const detailText = refLines.slice(1).map(l => l.trimStart()).filter(Boolean).join(' ')
        entries.push({ lines: refLines, fingerprint: _normalizeForDedup(detailText), isDup: false, dupOf: -1 })
      }
  ...
  // ── Pass 2: Dedup — check each entry against all prior entries ──
  // If near-duplicate found, mark and skip. First occurrence always wins.
  let dupCount = 0
  for (let i = 1; i < entries.length; i++) {
    for (let j = 0; j < i; j++) {
      if (entries[j].isDup) continue  // skip already-deduped entries as comparison targets
      const sim = _trigramSimilarity(entries[i].fingerprint, entries[j].fingerprint)
      if (sim >= 0.80) {
  ...
  // ── Pass 3: Render unique entries only ──
  let html = '<div class="ebm-refs">'
  if (headerText) {
    html += `<div class="ebm-refs-hdr">${esc(headerText)}</div>`
  }
  for (const entry of entries) {
    if (entry.isDup) continue
    html += _renderRefEntry(entry.lines, esc)
  }
  // Show dedup note if any were removed
  if (dupCount > 0) {
    html += `<div class="ebm-ref-dedup-note">（已自动去重 ${dupCount} 条高度相似的参考文献，保留首次出现）</div>`
  }
  html += '</div>'
  return html
}

function _renderRefEntry(lines, esc) {
  if (lines.length === 0) return ''
  let h = '<div class="ebm-ref-entry">'
  // First line: [N] PMID: xxx | 证据等级 X
  h += `<div class="ebm-ref-idx">${esc(lines[0])}</div>`
  // Subsequent lines (Title, Guidelines, etc.) — indented
...
/**
 * Parse EBM reference text into structured entries for renumbering.
 * Each entry: { content: string } — the full reference text WITHOUT the [N] prefix.
 * Used to merge EBM refs after KG evidence with continued numbering.
 */
function parseEbmRefEntries(refsText) {
  if (!refsText) return []
  ...
  for (const line of lines) {
    ...
    // New entry starts with [N]
    if (/^\[\d+\]/.test(trimmed)) {
      if (inEntry && curLines.length > 0) {
        entries.push({ content: curLines.join('\n').trim() })
      }
      // Strip the [N] prefix for renumbering
...
/**
 * Build HTML for EBM reference entries with offset numbering.
 * @param {Array}  entries   parsed entries from parseEbmRefEntries (each has .content)
 * @param {number} offset    starting index offset (e.g., KG count = 6, so first EBM ref = 7)
 */
function buildEbmRefsOffsetHtml(entries, offset) {
  ...
  entries.forEach((ref, i) => {
    ...
    // Split content into lines: first line gets the [N] prefix, rest indented
